...
<Aside type="note" title="Simplified for this walkthrough">
  The actual TypeScript Starter scaffold includes additional transforms (`dimensionToRem`,
  `fontWeightToNumber`) and a second CSS output using the `modifier` preset for per-theme files.
  Below is a simplified version to focus on the core concepts.
</Aside>
