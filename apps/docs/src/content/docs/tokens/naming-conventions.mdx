...
Tokens are organized into three tiers. Each tier has a distinct purpose, its own naming convention, and a clear dependency direction: higher tiers reference lower tiers, never the reverse. Every tier starts with the token **category** (`color`, `spacing`, `typography`, `shadow`).
...
Component tokens reference semantic tokens. Semantic tokens reference base tokens. This separation means you can re-theme an entire system by swapping semantic values, and every component updates automatically.

<Aside type="tip" title="Start simple">
Most token systems only need base and semantic tiers. Add component tokens when you find yourself repeatedly mapping the same semantic tokens to the same UI patterns across multiple components or platforms.
</Aside>
...
```json
{
  "color": {
    ...
    "input": {
      "border": {
        "$type": "color",
        "$root": { "$value": "{color.border.default}" },
        "focus": { "$value": "{color.border.default.focus}" }
      }
    }
  },
  "spacing": {
