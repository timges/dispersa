...
```typescript
import { build, css, tailwind, ios, android } from 'dispersa'
...
<Aside type="caution" title="iOS and Android are experimental">
  The `ios()` and `android()` output targets are experimental. Their APIs and generated code may
...
<Aside type="note" title="Per-output transforms and filters">
  Each output can have its own transforms and filters. Web outputs typically use kebab-case, native
  outputs use camelCase.
</Aside>
