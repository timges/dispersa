...
import { build, resolveTokens, resolveAllPermutations, css } from 'dispersa'
...
```typescript
const tokens = await resolveTokens(resolver, { theme: 'light' })
...
```typescript
const allPermutations = await resolveAllPermutations(resolver)
