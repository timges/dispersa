...
- **Maintainability** -- Detect deprecated tokens and prevent layering violations
...
const result = await dispersa.lint('./tokens.resolver.json', {
...
Dispersa includes six built-in rules:
...
| `no-going-back`       | Enforce semantic layering (base → semantic → component) |
